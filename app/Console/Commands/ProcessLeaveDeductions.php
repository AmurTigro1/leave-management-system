...
class ProcessLeaveDeductions extends Command
{
    ...
    public function handle()
    {
        ...
        Log::info("🔄 Starting leave deduction for {$thisMonth}");
        $records = TimeManagement::whereMonth('date', now()->month)
        ...
        foreach ($monthlyRecords as $userId => $userRecords) {
            ...
            MonthlySummary::updateOrCreate(
                ['month' => $thisMonth, 'user_id' => $userId],
                [
            ...
            if ($user) {
                ...
                if ($leaveDeduction > 0) {
                    ...
                    LeaveLog::create([
                        ...
                        'reason' => "Monthly late/undertime deduction for {$thisMonth}",
        ...
        Log::info("✅ Leave deductions have been processed for the month.");
