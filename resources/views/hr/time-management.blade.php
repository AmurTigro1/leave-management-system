                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="flex justify-center gap-4 items-center">
                                            ...
                                            <form action="{{ route('hr.time-management.destroy', $record->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this record?');">
                                                @csrf
                                                @method('DELETE')
    ...
    @endif
              ...
              <tbody>
                  @foreach($user->leaveLogs()->orderByDesc('created_at')->get() as $log)
                      <tr>
                          ...
                          <td class="p-2">
                              <form method="POST" action="{{ route('hr.leave-logs.delete', $log->id) }}" onsubmit="return confirm('Are you sure you want to delete this log?');">
                                  @csrf
                                  @method('DELETE')
        ...
        <div x-show="open" x-data="timeTracking()" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div class="bg-white w-full max-w-lg p-6 rounded-lg shadow-lg">
                ...
                <!-- Whole Day Form -->
                <form action="{{ route('hr.time-management.store') }}" method="POST" class="mt-4" id="whole-day-form" x-data="timeCalculator()">
                    @csrf
                ...
                <!-- Morning Form -->
                <form action="{{ route('hr.morning-time.management.store') }}" method="POST" class="mt-4" id="morning-form">
                    @csrf
                    <div class="grid grid-cols-2 gap-4">
                        <div class="col-span-2">
                            <label class="block text-sm font-medium text-gray-700">Date</label>
                ...
                <!-- Afternoon Form -->
                <form action="{{ route('hr.afternoon-time.management.store') }}" method="POST" class="mt-4" id="afternoon-form">
                    @csrf
