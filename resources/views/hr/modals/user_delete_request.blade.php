                ...
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-xl font-bold text-white">
                            Confirm Deletion #{{ $user->id }}
                        </h3>
                        <p class="text-rose-100 text-sm mt-1">
                            Are you sure you want to delete this overtime request?
                        </p>
                    </div>
                    <button onclick="closedeleteUserModaL({{ $user->id }})" class="text-white/80 hover:text-white transition">
...
<script>
    ...
    document.addEventListener('click', function (event) {
        document.querySelectorAll('[id^="deleteUserModaL"]').forEach(modal => {
