                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-2xl sm:text-3xl font-semibold text-gray-900">
                            {{ $groupedHolidays->flatten()->count() }}</p>
                    </div>
            ...
            <div class="bg-white p-4 sm:p-6 rounded-lg shadow-sm border border-gray-100">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-2xl sm:text-3xl font-semibold text-gray-900">
                            {{ $groupedHolidays->flatten()->where('type', 'regular')->count() }}</p>
                    </div>
                    <div class="p-2 sm:p-3 rounded-lg text-green-600">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 sm:h-8 w-6 sm:w-8" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M16 7V4a1 1 0 00-1-1H9a1 1 0 00-1 1v3m4 4v8m-7-4h14" />
                        </svg>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white p-4 sm:p-6 rounded-lg shadow-sm border border-gray-100">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-2xl sm:text-3xl font-semibold text-gray-900">
                            {{ $groupedHolidays->flatten()->where('type', 'special')->count() }}</p>
                    </div>
            ...
            <div class="bg-white p-4 sm:p-6 rounded-lg shadow-sm border border-gray-100">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-2xl sm:text-3xl font-semibold text-gray-900">
                            {{ $groupedHolidays->flatten()->where('type', 'national')->count() }}</p>
                    </div>
                    <div class="p-2 sm:p-3 rounded-lg text-red-600">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 sm:h-8 w-6 sm:w-8" viewBox="0 0 512 512"
                            fill="none">
        ...
        <div class="bg-white rounded-lg sm:rounded-xl shadow-md overflow-hidden mb-6 sm:mb-8 border border-gray-100">
            <div class="bg-gradient-to-r from-blue-600 to-blue-500 px-4 sm:px-6 py-3 sm:py-4">
                <div class="flex items-center justify-between">
                    ...
                    <span
                        class="inline-flex items-center px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-xs font-medium bg-blue-700 text-white">
                        {{ $selectedYear }}
                    </span>
                </div>
            </div>
            ...
            <div class="p-4 sm:p-6 relative overflow-visible">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                    @foreach ($calendarData as $month => $monthData)
                        <div
                            class="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow relative">
                            ...
                            <div class="p-2 sm:p-3">
                                <div class="grid grid-cols-7 gap-1 mb-1">
                                    @foreach (['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'] as $day)
                                        <div
                                            class="text-xs font-medium text-center py-1
                                ...
                                <div class="grid grid-cols-7 gap-1 relative overflow-visible">
                                    ...
                                    @for ($day = 1; $day <= $daysInMonth; $day++)
                                        ...
                                        <div class="relative group h-8 sm:h-10 flex items-center justify-center text-xs sm:text-sm font-medium
...
? 'right' : 'center';
                                        "
                                            ...
                                            @if ($hasHoliday)
                                                <span
                                                    class="absolute bottom-0.5 right-0.5 w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-blue-500"></span>
                                                ...
                                                <div x-show="showTooltip"
                                                    ...
                                                    style="display: none;">
                                                    <strong
                                                        class="block">{{ \Carbon\Carbon::create($monthData['year'], $month, $day)->format('F j, Y') }}</strong>
            ...
            <div class="p-4 sm:p-6">
                @if ($groupedHolidays->isEmpty())
                    <div class="text-center py-6 sm:py-8">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-10 sm:h-12 w-10 sm:w-12 mx-auto text-gray-400"
                            ...
                            <div>
                                <h3 class="text-base sm:text-lg font-medium text-gray-900 mb-2 sm:mb-4 flex items-center">
                                    <svg xmlns="http://www.w3.org/2000/svg"
                                        class="h-4 sm:h-5 w-4 sm:w-5 mr-2 text-blue-500" viewBox="0 0 20 20"
                                        fill="currentColor">
                                                ...
                                                <tr class="{{ $isSunday ? 'bg-red-50' : 'hover:bg-gray-50' }}">
                                                    <td class="px-3 sm:px-6 py-3 whitespace-nowrap">
                                                        <div class="flex items-center">
                                                            <div
                                                                class="text-sm font-medium {{ $isSunday ? 'text-red-600' : 'text-gray-900' }}">
                                                                {{ \Carbon\Carbon::parse($holiday->date)->format('jS F, Y') }}
                                                            </div>
                                                            @if ($isSunday)
                                                                <span
                                                                    class="ml-1 sm:ml-2 px-1 sm:px-2 inline-flex text-xs leading-4 sm:leading-5 font-semibold rounded-full bg-red-100 text-red-800">
                                                                    Sun
                                                                </span>
                                                            @endif
                                                        </div>
                                                    ...
                                                    <td class="px-3 sm:px-6 py-3 whitespace-nowrap">
                                                        <span
                                                            class="px-2 inline-flex text-xs leading-4 sm:leading-5 font-semibold rounded-full
                                                        {{ $holiday->type == 'regular' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800' }}">
                                                            {{ ucfirst($holiday->type) }}
                                                        </span>
                                                    </td>
                                                    <td
                                                        class="px-3 sm:px-6 py-3 whitespace-nowrap text-xs sm:text-sm text-gray-500">
                                                        @if ($holiday->repeats_annually)
                                                            <span class="inline-flex items-center text-green-600">
                                                                <svg xmlns="http://www.w3.org/2000/svg"
                                                                    class="h-3 sm:h-4 w-3 sm:w-4 mr-1" viewBox="0 0 20 20"
                                                                    fill="currentColor">
                                                                    <path fill-rule="evenodd"
