    ...
    <div id="leaveModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 hidden flex justify-center items-center z-[9999]">
        <div class="bg-white w-full max-w-lg rounded-lg shadow-xl p-6 relative">
            ...
            <button id="closeModalBtn"
                class="absolute top-3 right-4 text-gray-600 hover:text-gray-900 text-2xl">&times;</button>

            <h2 class="text-xl font-bold text-center mb-4">HR/Supervisor Information</h2>

            @foreach ($officials as $official)
                <form action="{{ route('hr-supervisor-info.update', $official->id) }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    @method('PUT')

                    <p class="text-sm border-yellow-600 border-l-4 bg-yellow-100 text-yellow-900 py-2 px-4 rounded-lg mb-2">
                        More than 10 days leave</p>
                    <div class="mb-4">
                ...
                <div class="mb-[25px]">
                    ...
                    <div class="flex justify-center gap-2">
                        ...
                        <button type="button" id="closeModalBtn2"
                            class="bg-gray-600 text-white py-2 px-4 rounded-lg">Cancel</button>
                    </div>
                </form>
            @endforeach
        </div>
                        ...
                        <div class="bg-white shadow-md rounded-lg p-4 lg:p-6 transition-all hover:shadow-lg">
                            ...
                            <div class="mt-4 flex flex-col sm:flex-row gap-4">
                                ...
                                <div class="flex-grow">
                                    ...
                                    <p class="text-gray-600 text-sm">Duration: <span
                                            class="font-semibold">{{ $leave->days_applied }} days</span></p>
                                </div>
                                <a href="{{ route('hr.leave_details', ['id' => $leave->id]) }}"
                                    class="text-blue-600 text-sm lg:text-base sm:self-end">View Request</a>
                            </div>

                            @if ($leave->leave_type == 'Mandatory Leave' && $leave->supervisor_status == 'rejected')
                                <div class="mt-3 p-3 bg-red-50 rounded-lg">
                                    <p class="font-bold text-sm lg:text-base">Supervisor Status: <span
                                            class="text-red-500 capitalize">{{ $leave->supervisor_status }}</span></p>
                                    <p class="text-gray-600 text-sm">Reason: <span
                                            class="text-red-500">{{ $leave->disapproval_reason }}</span></p>
                                </div>
                            @endif
                        </div>
                    @endforeach
            ...
            @else
                ...
                <div class="space-y-4">
                    @foreach ($ctoApplications as $cto)
                        @if (
                            ($cto->status == 'pending' && $cto->admin_status == 'Ready for Review') ||
                                (($cto->status != 'approved' || $cto->admin_status != 'Ready for Review') && $cto->hr_status != 'rejected'))
                            ...
                            <div class="bg-white shadow-md rounded-lg p-4 lg:p-6 transition-all hover:shadow-lg">
                                ...
                                <div class="mt-4 flex flex-col sm:flex-row gap-4">
                                    ...
                                    <div class="flex-grow">
                                        ...
                                        <div class="mt-2">
                                            <p class="text-gray-600 text-sm">Hours Applied:
                                    ...
                                    <a href="{{ route('hr.cto_details', ['id' => $cto->id]) }}"
                                        class="text-blue-600 text-sm lg:text-base sm:self-end">View Request</a>
                                </div>
                            </div>
                        @endif
