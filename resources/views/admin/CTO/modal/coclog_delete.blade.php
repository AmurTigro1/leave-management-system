<!-- Delete CTO Modal -->
<div id="deleteLogModal{{ $log->id }}" class="hidden fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-[9999] overflow-y-auto">
    <div class="w-full max-w-md bg-white rounded-xl shadow-2xl overflow-hidden" onclick="event.stopPropagation()">
        <div class="bg-gradient-to-r from-red-600 to-rose-700 px-6 py-5">
            <div class="flex items-center justify-between">
                ...
                <button onclick="closeDeleteLogModal({{ $log->id }})" class="text-white/80 hover:text-white transition">
        ...
        <div class="p-6">
            <form action="{{ route('coc-logs.destroy', $log->id) }}" method="POST">
                ...
                <p class="text-gray-700 text-sm mb-6">
                    This action will permanently delete the log entry. This cannot be undone.
                </p>

                <div class="flex justify-end space-x-3">
                    <button type="button"
                            onclick="closeDeleteLogModal({{ $log->id }})"
...
<script>
function openDeleteLogModal(overtimeId) {
    const modal = document.getElementById('deleteLogModal' + overtimeId);
    modal.classList.remove('hidden');
}

function closeDeleteLogModal(overtimeId) {
    const modal = document.getElementById('deleteLogModal' + overtimeId);
