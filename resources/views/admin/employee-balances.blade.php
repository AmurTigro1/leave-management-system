...
@section('content')
            ...
            <div class="p-4 sm:p-6">
                ...
                @if (session('success'))
                    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000)"
                        class="p-3 sm:p-4 mb-4 text-sm sm:text-base text-green-700 bg-green-100 border border-green-500 rounded">
                        {{ session('success') }}
                    </div>
                @endif
                ...
                @if ($errors->any())
                    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000)"
                        class="p-3 sm:p-4 mb-4 text-sm sm:text-base text-red-700 bg-red-100 border border-red-500 rounded">
                        <ul class="space-y-1">
                ...
                <!-- Header Section -->
                <div
                    class="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 sm:mb-6 gap-3 sm:gap-4">
                ...
                <div class="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
                    ...
                    <div class="bg-white rounded-lg shadow p-3 sm:p-4 border-l-4 border-green-500">
                        ...
                        <div class="text-xl sm:text-2xl font-bold text-gray-800 mt-1">
                            {{ number_format($employees->avg('vacation_leave_balance'), 2) }} days</div>
                    </div>
                    <div class="bg-white rounded-lg shadow p-3 sm:p-4 border-l-4 border-yellow-500">
                        ...
                        <div class="text-xl sm:text-2xl font-bold text-gray-800 mt-1">
                            {{ number_format($employees->avg('sick_leave_balance'), 2) }} days</div>
                    </div>
                ...
                <!-- Search Results -->
                <div id="employee-balances" class="overflow-x-auto">
    ...
    <div id="editModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 py-6 text-center sm:block sm:p-0">
            ...
            <!-- Center helper for vertical alignment -->
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>

            <!-- Modal panel -->
            <div
                class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all w-full max-w-3xl sm:my-8 sm:align-middle">
                    ...
                    <form id="balanceForm" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    <label for="vacation_leave_balance"
                                        class="block text-sm font-medium text-gray-700">Vacation Leave (days)</label>
                                    <input type="number" step="0.01" name="vacation_leave_balance"
                                        id="vacation_leave_balance"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                </div>

                                <div>
                                    <label for="mandatory_leave_balance"
                                        class="block text-sm font-medium text-gray-700">Mandatory Leave (days)</label>
                                    <input type="number" step="0.01" name="mandatory_leave_balance"
                                        id="mandatory_leave_balance"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                </div>


                                <div>
                                    <label for="sick_leave_balance" class="block text-sm font-medium text-gray-700">Sick
                                        Leave (days)</label>
                                ...
                                <div>
                                    <label for="wellness_leave_balance"
                                        class="block text-sm font-medium text-gray-700">Wellness
                                        Leave (days)</label>
                                    <input type="number" step="0.01" name="wellness_leave_balance"
                                        id="wellness_leave_balance"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                </div>
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    <label for="maternity_leave" class="block text-sm font-medium text-gray-700">Maternity
                                        Leave (days)</label>
                                ...
                                <div>
                                    <label for="paternity_leave" class="block text-sm font-medium text-gray-700">Paternity
                                        Leave (days)</label>
                                ...
                                <div>
                                    <label for="solo_parent_leave" class="block text-sm font-medium text-gray-700">Solo
                                        Parent Leave (days)</label>
                                    <input type="number" name="solo_parent_leave" id="solo_parent_leave"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                </div>

                                <div>
                                    <label for="study_leave" class="block text-sm font-medium text-gray-700">Study Leave
                                        (days)</label>
                                ...
                                <div>
                                    <label for="vawc_leave" class="block text-sm font-medium text-gray-700">VAWC Leave
                                        (days)</label>
                            ...
                            <div class="md:col-span-2 space-y-4">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div>
                                        <label for="rehabilitation_leave"
                                            class="block text-sm font-medium text-gray-700">Rehabilitation Leave
                                            (days)</label>
                                    ...
                                    <div>
                                        <label for="special_leave_benefit"
                                            class="block text-sm font-medium text-gray-700">Special Leave Benefit
                                            (days)</label>
                                    ...
                                    <div>
                                        <label for="special_privilege_leave"
                                            class="block text-sm font-medium text-gray-700">Special Privilege Leave
                                            (days)</label>
                                    ...
                                    <div>
                                        <label for="special_emergency_leave"
                                            class="block text-sm font-medium text-gray-700">Special Emergency Leave
                                            (days)</label>
    ...
    <script>
        ...
        function openEditModal(employee) {
            ...
            document.getElementById('wellness_leave_balance').value = employee.wellness_leave_balance;
