                            ...
                            <div class="bg-white shadow-md rounded-lg p-4 md:p-6 transition-all hover:shadow-lg">
                                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span
                                            class="bg-yellow-500 text-white py-1 px-3 rounded-full text-sm">{{ $leave->status }}</span>
                                    </div>
                                </div>
                                ...
                                <div class="flex flex-col sm:flex-row items-end gap-4">
                                    <div class="flex-shrink-0">
                                        @if ($leave->user->profile_image)
                                            @php
                                                ...
                                                if ($leave->user->profile_image) {
                                                    $imagePath1 =
                                                        'storage/profile_images/' . $leave->user->profile_image;
                                                    $imagePath2 =
                                                        'storage/profile_pictures/' . $leave->user->profile_image;
                                    ...
                                    <div class="flex-grow">
                                        <h3 class="text-sm sm:text-base font-semibold text-gray-900 mb-1 uppercase">
                                            ...
                                            {{ strtoupper(substr($leave->user->middle_name, 0, 1)) }}.
                                            {{ $leave->user->last_name }}
                                        </h3>
                                        <p class="text-gray-600 text-xs sm:text-sm mb-1">Leave Type:
                                            {{ $leave->leave_type }}</p>
                                        <p class="text-gray-600 text-xs sm:text-sm">Duration: <span
                                                class="font-semibold">{{ $leave->days_applied }} days</span></p>
                                    </div>
                                    <a href="{{ route('admin.leave_details', ['id' => $leave->id]) }}"
                                        class="text-blue-600 text-sm sm:text-base">View Request</a>
                                </div>
                            </div>
                        @endforeach
                    </div>
                            ...
                            <div class="bg-white shadow-md rounded-lg p-4 md:p-6 transition-all hover:shadow-lg">
                                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span
                                            class="bg-yellow-500 text-white py-1 px-3 rounded-full text-sm">{{ $cto->status }}</span>
                                    </div>
                                </div>
                                ...
                                <div class="flex flex-col sm:flex-row items-end gap-4">
                                        ...
                                        @if ($cto->user->profile_image)
                                            @php
                                                ...
                                                if ($cto->user->profile_image) {
                                                    ...
                                                    $imagePath2 =
                                                        'storage/profile_pictures/' . $cto->user->profile_image;
                                    ...
                                    <div class="flex-grow">
                                        <h3 class="text-sm sm:text-base font-semibold text-gray-900 mb-1 uppercase">
                                            {{ $cto->user->first_name }}
                                            {{ strtoupper(substr($cto->user->middle_name, 0, 1)) }}.
                                            {{ $cto->user->last_name }}
                                        </h3>
                                        <p class="text-gray-600 text-xs sm:text-sm mb-1">Working Hours:
                                            {{ $cto->working_hours_applied }} hours</p>
                                        <p class="text-gray-600 text-xs sm:text-sm">
                                            Duration: {{ count(explode(', ', $cto->inclusive_dates)) }}
                                            {{ Str::plural('day', count(explode(', ', $cto->inclusive_dates))) }}
                                        </p>
                                    </div>
                                    <a href="{{ route('admin.cto_details', ['id' => $cto->id]) }}"
                                        class="text-blue-600 text-sm sm:text-base">View Request</a>
                                </div>
                            </div>
                        @endforeach
                        ...
                        <div class="bg-white shadow rounded p-3">
                            <div class="flex items-center gap-3 mb-2">
                                @if ($leave->user->profile_image)
                                    ...
                                    <img src="{{ $profileImage ?? asset('img/default-avatar.png') }}"
                                        class="w-10 h-10 rounded-full object-cover" alt="{{ $leave->user->name }}">
                                @else
                                    <img src="{{ asset('img/default-avatar.png') }}" alt=""
                                        class="w-10 h-10 rounded-full object-cover">
                                @endif
                                <div>
                                    <h3 class="text-sm font-semibold uppercase">{{ $leave->user->first_name }}
                                        {{ strtoupper(substr($leave->user->middle_name, 0, 1)) }}.
                                        {{ $leave->user->last_name }}</h3>
                            ...
                            <div class="flex items-center justify-between text-sm">
                                <span
                                    class="bg-yellow-500 text-white py-1 px-2 rounded-full text-xs">{{ $leave->status }}</span>
                                <a href="{{ route('admin.leave_details', ['id' => $leave->id]) }}"
                        ...
                        <div class="bg-white shadow rounded p-3">
                            <div class="flex items-center gap-3 mb-2">
                                @if ($cto->user->profile_image)
                                    ...
                                    <img src="{{ $profileImage ?? asset('img/default-avatar.png') }}"
                                        class="w-10 h-10 rounded-full object-cover" alt="{{ $cto->user->name }}">
                                @else
                                    <img src="{{ asset('img/default-avatar.png') }}" alt=""
                                        class="w-10 h-10 rounded-full object-cover">
                                @endif
                                <div>
                                    <h3 class="text-sm font-semibold uppercase">{{ $cto->user->first_name }}
                                        {{ strtoupper(substr($cto->user->middle_name, 0, 1)) }}.
                                        {{ $cto->user->last_name }}</h3>
                                    <p class="text-xs text-gray-500">Hours: {{ $cto->working_hours_applied }}</p>
                                    <p class="text-xs text-gray-500">Duration:
                                        {{ count(explode(', ', $cto->inclusive_dates)) }}
                                        {{ Str::plural('day', count(explode(', ', $cto->inclusive_dates))) }}</p>
                                </div>
                            </div>
                            <div class="flex items-center justify-between text-sm">
                                <span
                                    class="bg-yellow-500 text-white py-1 px-2 rounded-full text-xs">{{ $cto->status }}</span>
                                <a href="{{ route('admin.cto_details', ['id' => $cto->id]) }}"
                                    class="text-blue-600">View</a>
                            </div>
                        </div>
