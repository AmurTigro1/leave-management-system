...
>
    ...
    <nav class="mt-6">
        <a href="{{ route('employee.dashboard') }}" 
           ...
           {{ request()->routeIs('employee.dashboard') || request()->routeIs('profile.edit') ? 'text-white bg-blue-500' : 'text-gray-600' }}">
            Dashboard
        </a>
        <a href="{{ route('employee.make_request')}}" 
           class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200 hover:text-blue-500 hover:bg-blue-100 
           {{ request()->routeIs('employee.make_request')  || request()->routeIs('boarding_house.edit') || request()->routeIs('rooms.edit') || request()->routeIs('boarding_house-rooms.index') ? 'text-white bg-blue-500' : 'text-gray-600' }}">
            Make a Request 
        </a>
        <a href="{{ route('employee.leave_request')}}" 
           class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200 hover:text-blue-500 hover:bg-blue-100 
           {{ request()->routeIs('employee.leave_request') || request()->routeIs('employee.leave_show') ? 'text-white bg-blue-500' : 'text-gray-600' }}">
            My Request
        </a>
        <a href="{{ route('holiday.calendar') }}" 
        class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200 hover:text-blue-500 hover:bg-blue-100 
        {{ request()->routeIs('holiday.calendar') ? 'text-white bg-blue-500' : 'text-gray-600' }}">
         Holidays
     </a>
    </nav>
</div>
