        ...
        <nav class="flex items-center space-x-6">
            ...
            @if (Auth::check())
                <div class="relative w-full sm:w-auto">
                    <button id="dropdown-btn" class="flex items-center justify-between sm:w-auto bg-white px-4 py-2 rounded-lg shadow-md border border-gray-300 transition-all duration-200 ease-in-out">
                            ...
                            <div class="w-10 h-10 rounded-full overflow-hidden bg-gray-100 flex items-center justify-center border-2 border-gray-200 hover:border-blue-300 transition-all duration-300">
                                ...
                                @else
                                    <svg class="w-6 h-6 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4z"></path>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"></path>
                                    </svg>
                                @endif
                            </div>
                            <!-- Username -->
                    ...
                    <div id="dropdown-menu" class="absolute hidden bg-white shadow-lg rounded-lg mt-2 w-44 right-0 z-50 border border-gray-200">
                        <ul class="text-gray-700 text-sm space-y-2 py-2">
                            <!-- Balances (Only Visible on Mobile) -->
                            <li class="sm:hidden block px-4 text-gray-600">
                                <span class="font-medium text-xs">Sick Leave:</span>
                                <span class="font-semibold text-gray-800 text-xs bg-gray-50 px-2 py-1 rounded-md ml-2">
                                    {{ Auth::user()->sick_leave_balance }}
                                </span>
                            </li>
                            <li class="sm:hidden block px-4 text-gray-600">
                                <span class="font-medium text-xs">Vacation Leave:</span>
                                <span class="font-semibold text-gray-800 text-xs bg-gray-50 px-2 py-1 rounded-md ml-2">
                                    {{ Auth::user()->vacation_leave_balance }}
                                </span>
                            </li>
                            <li class="sm:hidden block px-4 text-gray-600">
                                <span class="font-medium text-xs">Total COCs:</span>
                                <span class="font-semibold text-gray-800 text-xs bg-gray-50 px-2 py-1 rounded-md ml-2">
                                    {{ Auth::user()->overtime_balance }}
                                </span>
                            </li>
                            ...
                            <li>
                                <button id="openModal" class="block w-full text-left px-4 py-2 hover:bg-red-100 hover:text-red-600">Logout</button>
                            </li>
                        </ul>
                    </div>
                </div>
