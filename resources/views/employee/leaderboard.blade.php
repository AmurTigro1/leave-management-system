...
@section('content')
    <div class="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 w-full max-w-3xl mx-auto animate-fade-in">
        ...
        <p class="text-center text-gray-600 text-sm sm:text-base mb-4 sm:mb-6">
            Recognizing our most dedicated employees! This leaderboard highlights those with the <strong>fewest
                absences</strong> in the past 30 days.
        </p>

        <div class="flex justify-center mb-4 sm:mb-6">
            <button
                class="bg-gray-900 text-white px-3 py-1 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-semibold focus:ring">Last
                30 days</button>
        </div>
                ...
                <div class="flex items-center justify-between p-3 sm:p-4 bg-gray-50 rounded-lg">
                    ...
                    <div class="text-right flex-shrink-0 ml-2">
                        @if ($employee->total_absences == 0)
                            <span
                                class="px-2 py-1 bg-green-500 text-white text-xs font-semibold rounded-lg whitespace-nowrap">Perfect
                                🎯</span>
                        @elseif ($employee->total_absences <= 2)
                            <span
                                class="px-2 py-1 bg-blue-500 text-white text-xs font-semibold rounded-lg whitespace-nowrap">Excellent
                                🎖️</span>
                        @else
                            <span
                                class="px-2 py-1 bg-gray-500 text-white text-xs font-semibold rounded-lg whitespace-nowrap">Good
                                👍</span>
                        @endif
        ...
        <div class="text-center mt-4 sm:mt-6">
            <button
                ...
                hx-get="{{ route('users.modal') }}" hx-target="#modal-content" hx-trigger="click">
                View everyone
            </button>
        </div>

        <div id="modal"
            class="fixed inset-0 bg-gray-900 bg-opacity-50 hidden flex justify-center items-center z-[9999] p-4">
