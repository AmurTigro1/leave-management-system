    ...
    <div class="max-w-7xl mx-auto p-6 space-y-6 animate-fade-in">
        <div class="container mx-auto px-2 sm:px-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                @foreach ([['Leave Requests', $leaveStats], ['CTO Requests', $cocStats]] as [$title, $stats])
                    <div class="bg-white p-4 sm:p-6 rounded-lg shadow-md flex flex-col h-full">
                        <h2 class="text-lg sm:text-xl md:text-2xl font-bold text-gray-800 mb-3 sm:mb-4">{{ $title }}
                        </h2>
                        <div class="grid grid-cols-3 gap-2 sm:gap-4 flex-grow">
                            @foreach (['Pending' => 'yellow', 'Approved' => 'green', 'Rejected' => 'red'] as $status => $color)
                                <div
                                    class="bg-{{ $color }}-100 p-3 sm:p-4 md:p-6 rounded-lg shadow text-center flex flex-col justify-center min-h-[100px] sm:min-h-[120px] md:min-h-[140px]">
                                    <h3 class="text-xs sm:text-sm md:text-base font-semibold text-gray-700">
                                        {{ $status }}</h3>
                                    <p class="text-xl sm:text-2xl md:text-3xl font-bold text-{{ $color }}-600 mt-1">
                                        {{ $stats[$status] }}</p>
                                </div>
                            @endforeach
                        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mt-4 sm:mt-6">
                ...
                <div class="bg-white p-4 sm:p-6 rounded-lg shadow-md">
                    <h3 class="text-lg sm:text-xl font-semibold text-gray-700 mb-3 sm:mb-4">CTO / Overtime Requests Overview
                    </h3>
                    <div class="w-full h-48 sm:h-64">
            ...
            new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        data: [{{ $leaveStats['Pending'] }}, {{ $leaveStats['Approved'] }},
                            {{ $leaveStats['Rejected'] }}
                        ],
            ...
            new Chart(ctxCTO, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        data: [{{ $cocStats['Pending'] }}, {{ $cocStats['Approved'] }},
                            {{ $cocStats['Rejected'] }}
                        ],
                        backgroundColor: ['#FBBF24', '#34D399', '#EF4444'],
                        borderColor: ['#D97706', '#059669', '#B91C1C'],
