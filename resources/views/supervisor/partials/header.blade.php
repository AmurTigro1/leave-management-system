    ...
    <div class="container mx-auto max-w-7xl px-4 flex sm:flex-row justify-between space-y-3 sm:space-y-0">
        ...
        <nav class="flex items-center space-x-6">
            <div class="flex flex-row text-gray-600 space-x-2">
                ...
                <div class="relative">
                    <button id="supervisor-notification-button" class="p-2 rounded-full bg-gray-100 relative">
                        ...
                        @if(auth()->user()->unreadNotifications->count() > 0)
                            <span id="supervisor-notification-badge" class="absolute -top-1 -right-1 bg-red-500 text-white px-2 py-0.5 text-xs rounded-full">
                                {{ auth()->user()->unreadNotifications->count() }}
                            </span>
                        @endif
                    </button>
                
                    <div id="supervisor-notification-container" class="absolute right-0 bg-white shadow-lg rounded-lg sm:rounded-xl border border-gray-200 p-3 sm:p-4 mt-2 w-56 sm:w-64 md:w-72 hidden z-50">
                        <h3 class="text-sm sm:text-base font-semibold text-gray-700 mb-2">Supervisor Notifications</h3>

                        <div class="max-h-60 overflow-y-auto">
                            @forelse(auth()->user()->notifications as $notification)
                                <div class="notification-item p-2 rounded mb-2 bg-gray-50 hover:bg-gray-100 flex justify-between items-start sm:items-center" data-id="{{ $notification->id }}">
                                    <span class="text-xs sm:text-sm break-words">
                                        {!! Str::of($notification->data['message'] ?? 'New Notification')
                                            ->replace('approved', '<span class="text-green-500 font-medium">approved</span>') 
                                            ->replace('rejected', '<span class="text-red-500 font-medium">rejected</span>') !!}
                                    </span>
                                    <button class="delete-notification text-red-500 hover:text-red-700 text-xs px-1" data-id="{{ $notification->id }}">
                                        ✖
                                    </button>
                                </div>
                            @empty
                        ...
                        <div class="mt-3 flex gap-2 justify-between border-t border-gray-100 pt-3">
                            <button id="supervisor-mark-all-as-read" class="text-gray-500 hover:text-gray-700 text-xs sm:text-sm">Mark all as read</button>
                            <button id="supervisor-delete-all-notifications" class="text-gray-500 hover:text-gray-700 text-xs sm:text-sm">Delete All</button>
                        </div>
                    </div>
                </div>
            ...
            <div class="relative">
                ...
                <div id="dropdown-menu" class="absolute hidden bg-white shadow-lg rounded-lg mt-2 w-44 right-0 z-50 border border-gray-200">
                    <ul class="py-2 text-gray-700 text-sm">
                        <li>
                            <a href="/supervisor-profile" class="block px-4 py-2 hover:bg-blue-100 hover:text-blue-600 transition">Profile</a>
                        </li>
                        <li>
...
<script>
    ...
    const supervisorNotificationButton = document.getElementById("supervisor-notification-button");
    const supervisorNotificationContainer = document.getElementById("supervisor-notification-container");
    const supervisorMarkAsReadButton = document.getElementById("supervisor-mark-all-as-read");
    const supervisorDeleteAllButton = document.getElementById("supervisor-delete-all-notifications");
    const supervisorNotificationBadge = document.getElementById("supervisor-notification-badge");
    ...
    supervisorNotificationButton.addEventListener("click", function () {
        supervisorNotificationContainer.classList.toggle("hidden");
    });

    supervisorMarkAsReadButton.addEventListener("click", function () {
        fetch("{{ route('supervisor.notifications.markAsRead') }}", {
            method: "POST",
            headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                if (supervisorNotificationBadge) {
                    supervisorNotificationBadge.style.display = "none";
                }
            }
        })
    ...
    document.querySelectorAll(".delete-notification").forEach(button => {
        button.addEventListener("click", function () {
            ...
            fetch(`/notifications/supervisor-delete/${notificationId}`, {
                method: 'DELETE',
                headers: {
                    "X-CSRF-TOKEN": "{{ csrf_token() }}",
                    "Content-Type": "application/json"
    ...
    supervisorDeleteAllButton.addEventListener("click", function () {
        fetch("{{ route('supervisor.notifications.deleteAll') }}", {
            method: "DELETE",
            headers: {
                "X-CSRF-TOKEN": "{{ csrf_token() }}",
                "Content-Type": "application/json",
    ...
    document.addEventListener("click", function (event) {
        if (!supervisorNotificationButton.contains(event.target) && !supervisorNotificationContainer.contains(event.target)) {
            supervisorNotificationContainer.classList.add("hidden");
        }
    });
});
</script>
