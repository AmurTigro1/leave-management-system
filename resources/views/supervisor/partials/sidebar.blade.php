...
<nav class="mt-6 space-y-2">
    ...
    <a href="{{ route('supervisor.dashboard')}}" 
        class="m-3 flex items-center text-white px-4 py-3 text-sm rounded-lg transition-all duration-200 
            ...
            {{ request()->routeIs('supervisor.dashboard') ? 'bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-lg' : 'text-gray-600 ' }}">
    ...
    <a href="{{ route('supervisor.requests')}}" 
       class="m-3 flex items-center text-white px-4 py-3 text-sm rounded-lg transition-all duration-200 
              hover:bg-gradient-to-r hover:from-blue-500 hover:to-blue-600 hover:text-white 
              {{ request()->routeIs('supervisor.requests') ? 'bg-gradient-to-r from-blue-500 to-blue-600 text-white shadow-lg' : 'text-gray-600 ' }}">
        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
